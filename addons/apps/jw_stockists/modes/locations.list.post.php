...
<?php if (PerchUtil::count($locations)): ?>
    <table class="d">
        ...
        <tbody>
            <?php foreach($locations as $Location): ?>
                ...
                <td>
                    <?php echo $HTML->encode($Location->get_status()); ?>
                </td>
                <td>
                    <?php if($CurrentUser->has_priv('jw_stockists.delete')): ?>
                    <a href="<?php echo $HTML->encode($API->app_path()); ?>/delete/?id=<?php echo $HTML->encode(urlencode($Location->id())); ?>" class="delete inline-delete">
    ...
    <?php if ($Paging->enabled()) echo $HTML->paging($Paging); ?>
<?php endif; ?>